...
function inittitle(gtitle) {
    ...
    for (var a in gtitle) {
        ...
        if (gtitle[a] == "guide_commission" || gtitle[a] == "exclusive_commission") {
    ...
    if (list.length > 0) {
        for (var one in list) {
            ...
            for (var index in config) {
                ...
                else if (config[index] == "salary_include_show") {
                    if (key == "是") {
                        obj.salary_include = true;
                    } else if (key == "否") {
                        obj.salary_include = false;
                    }
...
function submitForm() {
    ...
    $.submitForm({
        url: "/ProductManage/CommissionInfo/Import?date=" + new Date(),
        param: data,
        success: function (data) {
            if (data.state == "success") {
                window.location.href = "/ProductManage/CommissionInfo/ImportHistory";
            }
        },
        error: function (data) {
            $.modalAlert(data.responseText, 'error');
