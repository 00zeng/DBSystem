...
var tableHeader = [
    {
        ...
        formatter: (function (value, row) {
            return "<a class='btn btn-primary btn-xs' href='/ProductManage/CommissionInfo/Show?id=" + row.id + " ' title='查看详情'><i class='fa fa-search'></i></a>";
        })
    },
    ...
    { field: "import_file", title: "导入文件名", align: "center", sortable: true },
    {
        ...
        formatter: function (value, row, index) {
            if (value < 0) return "<span class='label label-danger'>不通过</span>";
            if (value == 100) return "<span class='label label-success'>通过</span>";
            if (value == 0) return "<span class='label label-warning'>未审批</span>";
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/ProductManage/CommissionInfo/GetListHistory?date=" + new Date(),        //请求后台的URL（*）
            ...
            queryParams: function (params) {
                var param = {
                    ...
                    sord: params.order,
                    name: $('#name').val().trim(),
                    effect_status: $('#effect_status').val(),
