...
var emp_category;
//返利政策
var str1 = "<tr style='display: table;width: 100%;table-layout: fixed;'><td class='col-md-6'><div class='input-group'><input  class='form-control text-center' value='";
        ...
        success: function (data) {
            ...
            function RebateMode1() {
                ...
                $.each(data.rewardList, function (index, item) {
                    ...
                    tr += str1 + item.target_min + str2 + item.target_max + "' disabled><td class='col-md-6'><div class='input-group'> <div class='input-group-btn'><button type='button' class='btn btn-" + btnClass + "' name='rewards'>" + rewards + "</button></div><span  class='input-group-addon no-border'>"
                         + reward + "</span><span name='rebate_mode_input1' class='input-group-addon no-border no-padding'>" + str3 + "</span><span name='rebate_mode_input2' class='input-group-addon no-border no-padding'>" + str4 + "</span></div></td></tr>";
                })
            ...
            function ProductScope() {
                ...
                $.each(pSelList, function (index, item) {
                    ...
                    tr += "<tr style='display: table;width: 100%;table-layout: fixed;'><td style='text-align:center;vertical-align:middle;' class='col-md-3'>" + item.model + "</td><td style='text-align:center;vertical-align:middle;' class='col-md-2'>" + item.color + "</td><td style='text-align:center;vertical-align:middle;' class='col-md-2'>" + item.price_wholesale + "</td><td style='text-align:center;vertical-align:middle;' class='col-md-5'><div class='input-group'> <div class='input-group-btn'><button type='button' class='btn btn-" + btnClass + "' name='reward_p'>" + reward_p + "</button></div><span  class='input-group-addon no-border'>"
                    + reward + "</span><span name='rebate_mode_input1' class='input-group-addon no-border no-padding'>" + str3 + "</span><span name='rebate_mode_input2' class='input-group-addon no-border no-padding'>" + str4 + "</span></div></td></tr>";
                })
...
function submitForm() {
    ...
    $.submitForm({
        url: "/ActivityManage/Attaining/Approve?date=" + new Date(),
        param: data,
        success: function (data) {
            if (data.state == "success") {
                window.location.href = "/ActivityManage/Attaining/ApproveIndex";
            }
        },
        error: function (data) {
            $.modalAlert(data.responseText, 'error');
