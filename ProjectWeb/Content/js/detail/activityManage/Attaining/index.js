...
var tableHeader = [
     {
         ...
         formatter: function (value, row) {
             if (row.activity_status == -2 || row.activity_status == -1 || row.activity_status == 0)
                 return "<a class='btn btn-primary btn-xs' href='/ActivityManage/Attaining/Show?id=" + row.id + " ' title='查看活动详情'><i class='fa fa-search'></i></a>";
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/ActivityManage/Attaining/GetList?date=" + new Date(),         //请求后台的URL（*）
