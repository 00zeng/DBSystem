...
function getTotal() {
    $.ajax({
        url: "/ActivityManage/Calculate/GetTotalPK?emp_name=" + $('#name').val() + "&company_id=" + $('#company_id').val()
...
var tableHeader2 = [
    ...
    {
        field: "total_count", title: "累计销量", align: "center", sortable: true, width: 150,
        formatter: (function (value) { return ToThousandsStr(value); })
    },
    {
        field: "activity_target", title: "目标销量", align: "center", sortable: true, width: 150,
        formatter: (function (value) { return ToThousandsStr(value); })
    },
    {
        field: "total_ratio", title: "累计完成率(%)", align: "center", sortable: true, width: 150,
        formatter: (function (value) { return ToThousandsStr(value); })
    },
    {
        field: "total_reward", title: "活动奖金", align: "center", sortable: true, width: 150,
        formatter: (function (value) { return ToThousandsStr(value); })
    },

    {
        field: "winner", title: "输赢结果", align: "center", width: 120, formatter: function (cellvalue) {
            if (cellvalue == 1) {
                return "赢";
            }
            else
    ...
    {
        field: "end_date", title: "活动时间", align: "center", width: 180, sortable: true,
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/ActivityManage/Calculate/PKMainList?date=" + new Date(),        //请求后台的URL（*）
    ...
    oInit.InitSubTable = function (index, row, $detail) {
        ...
        $(cur_table).bootstrapTable({
            url: "/ActivityManage/Calculate/PKList",        //请求后台的URL（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            sortable: true,                     //是否启用排序
...
//重新查询  
function Export() {
    window.location.href = "/ActivityManage/Calculate/PKExport?company_id=" + $('#company_id').val()
            + "&emp_name=" + $('#name').val().trim()
