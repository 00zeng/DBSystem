...
function getTotal() {
    $.ajax({
        url: "/ActivityManage/Calculate/GetTotalSalesPer?emp_name=" + $('#name').val() + "&company_id=" + $('#company_id').val()
...
var tableHeader2 = [
     {
         ...
         formatter: function (value, row) {
             return "<a class='btn btn-primary btn-xs' href='/ActivityManage/SalesPerformance/SalePerfShowActivity?id=" + row.id + " ' title='查看活动详情'><i class='fa fa-search'></i></a>";
         }
     },
    {
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/ActivityManage/Calculate/PerfMainList?date=" + new Date(),        //请求后台的URL（*）
    ...
    oInit.InitSubTable = function (index, row, $detail) {
        ...
        $(cur_table).bootstrapTable({
            url: "/ActivityManage/Calculate/PerfList",        //请求后台的URL（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            sortable: true,                     //是否启用排序
...
//重新查询  
function Export() {
    window.location.href = "/ActivityManage/Calculate/PerfExport?company_id=" + $('#company_id').val()
            + "&emp_name=" + $('#name').val().trim()
