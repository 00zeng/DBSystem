...
function getTotal() {
    $.ajax({
        url: "/ActivityManage/Calculate/GetTotalRecom?emp_name="+ $('#name').val() + "&company_id=" + $('#company_id').val()
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/ActivityManage/Calculate/RecomMainList?date=" + new Date(),        //请求后台的URL（*）
    ...
    oInit.InitSubTable = function (index, row, $detail) {
        ...
        $(cur_table).bootstrapTable({
            url: "/ActivityManage/Calculate/RecomList",        //请求后台的URL（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            sortable: true,                     //是否启用排序
...
//重新查询  
function Export() {
    window.location.href = "/ActivityManage/Calculate/RecomExport?company_id=" + $('#company_id').val()
            + "&emp_name=" + $('#name').val().trim()
