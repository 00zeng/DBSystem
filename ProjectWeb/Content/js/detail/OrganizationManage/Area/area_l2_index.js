...
var tableHeader = [
    ...
    { field: "parent_name", title: "上级区域", align: "center", sortable: true, width: 150 },
...
function GetAreaL1List() {
    var cur_company = $("#company_id2").val();
    $("#area_l1_id").empty();
    if (cur_company > 0) {
...
function AjustL2() {
    ...
    if (idArray.length <= 0) {
        ...
        top.layer.confirm("确定要调整区域吗?", function (index) {
            var date = {};
            $.submitForm({
                url: "/OrganizationManage/Area/AjustL2",
                param: { idArray: idArray, effect_date: $("#effect_date2").val(), company_id: $("#company_id2").val(), area_l1_id: $("#area_l1_id").val() },
                success: function (data) {
                    if (data.state == "success") {
                        top.layer.closeAll();
