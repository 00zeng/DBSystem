...
$(function () {
    ...
    //    $.modalAlert("包销信息须由事业部人员导入", "error")
    //    window.history.go(-1);
    //    return;
    //}
    initTable();
...
//    $.ajax({
//        url: "/HumanResource/EmployeeManage/GetSalesList",
//        type: "get",
//        async: false,
//        dataType: "json",
//        success: function (data) {
...
function resoveresult(config, list) {
    ...
    if (list.length > 0) {
        for (var one in list) {
            ...
            //            resoveFail("第" + num + "行：系统中不存在经销商【" + distributorName + "】，请核查后重新导入！");
            //            return;
            //        }
            //        break;
            //    }
            //    cLen++;
            //    if (aLen == area_l2_list.length && cLen == distributorTree.length) {
            ...
            obj.check_status = 1;
            obj.main_id = guid;
            obj.refund = obj.price_wholesale - obj.price_exclusive;
            importData.push(obj);
            num++;
        }
...
function submitForm() {
    ...
    $.submitForm({
        url: "/SaleManage/Exclusive/Import?date=" + new Date(),
        param: data,
        success: function (data) {
            if (data.state == "success") {
                window.location.href = "/SaleManage/Exclusive/HistoryIndex";
            }
        },
        error: function (data) {
            $.modalAlert(data.responseText, 'error');
