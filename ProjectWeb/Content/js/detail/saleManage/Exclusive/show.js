...
$(function () {
    $.ajax({
        url: "/SaleManage/Exclusive/GetInfoMain?date=" + new Date(),
        type: "get",
        data: { id: id },
        dataType: "json",
        success: function (data) {
            ...
            $(".form").formSerializeShow(data.mainInfoStr[0]);
            $(".form").formSerializeShow(data.approveList[0]);

            initTable();

            var approveStatus = data.mainInfoStr[0].status;
            ...
            if (approveStatus == 0) {
                ...
                if (accJobid == data.mainInfoStr[0].creator_job_history_id)
...
var TableInit = function (columns) {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            url: '/SaleManage/Exclusive/GetInfoPage',
            //data: data,
            method: 'get',                      //请求方式（*）
...
function inittitle(gtitle) {
    ...
    for (var a in gtitle) {
        ...
        if (gtitle[a] == "price_wholesale" || gtitle[a] == "price_exclusive" || gtitle[a] == "guide_commission") {
...
function Show(obj) {
    ...
    window.location.href = "/SaleManage/Exclusive/Index?id=" + id + "&phone_sn=" + phone_sn;
}
...
//审批
function approval() {
    window.location.href = "/SaleManage/Exclusive/Approve?id=" + id;
}
...
function delForm() {
    top.layer.confirm("确认要撤回? ", function (index) {
        ...
        data["id"] = id;
        $.ajax({
            url: "/saleManage/Exclusive/Delete?id=" + id,
            type: "post",
            data: data,
            success: function (data) {
                top.layer.msg("撤回成功");
