...
function getTotal() {
    $.ajax({
        url: "/SaleManage/Summary/SalesManagerGetTotalInfo?emp_name=" + $('#name').val() + "&company_id=" + $('#company_id').val()
...
var tableHeader2 = [
    ...
    { field: "outstorage_time", title: "出库时间", align: "center", sortable: true, width: 150 },
     {
         field: "price_retail", title: "实销价格", align: "center", sortable: true, width: 120,
         formatter: (function (value) { return ToThousandsStr(value); })
     },
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/SaleManage/Summary/SalesManagerMainList?date=" + new Date(),        //请求后台的URL（*）
    ...
    oInit.InitSubTable = function (index, row, $detail) {
        ...
        $(cur_table).bootstrapTable({
            url: "/SaleManage/Summary/SalesManagerDetailList",        //请求后台的URL（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            sortable: true,                     //是否启用排序
...
//重新查询  
function Export() {
    window.location.href = "/SaleManage/Summary/ExportExcelSalesManagerDetail?company_id=" + $('#company_id').val()
            + "&emp_name=" + $('#name').val().trim()
