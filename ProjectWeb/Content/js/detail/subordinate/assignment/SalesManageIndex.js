...
var tableHeader = [
    {
        ...
        formatter: (function (value, row) {
            if (row.area_l1_id == 0) {
                return '<a class="btn btn-info btn-xs" href="/SubordinateManage/Assignment/SalesManageAdd?id=' + row.id + '&name=' + encodeURI(encodeURI(row.name)) + '">指派</a>';
            } else {
                return '<a class="btn btn-info btn-xs" href="/SubordinateManage/Assignment/SalesManageAdd?id=' + row.id + '&name=' + encodeURI(encodeURI(row.name)) + '">指派</a> '
...
function Remove(obj) {
    ...
    top.layer.confirm("您确认要解绑业务经理：“" + name + "”吗？", function (index) {
        if ($('[name=__RequestVerificationToken]').length > 0) {
            data["__RequestVerificationToken"] = $('[name=__RequestVerificationToken]').val();
        }
        data["emp_id"] = id;
        $.submitForm({
            url: "/SubordinateManage/Assignment/RemoveSalesManager?date=" + new Date(),
            param: data,
...
var TableInit = function (isAllGuide) {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/SubordinateManage/Assignment/SalesManagerList?date=" + new Date(),        //请求后台的URL（*）
            ...
            queryParams: function (params) {
                var param = {
                    ...
                    sidx: params.sort,
                    sord: params.order,
                    isAllSalesManger: $("input[id='ShowAll']").prop("checked"),
                };
                return param;
            },
