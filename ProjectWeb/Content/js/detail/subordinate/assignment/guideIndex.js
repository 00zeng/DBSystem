...
var tableHeader = [
    {
        ...
        formatter: (function (value, row) {
            if (row.d_list_name == null) {
                return '<a class="btn btn-info btn-xs" href="/SubordinateManage/Assignment/GuideAdd?id=' + row.id + '&name=' + encodeURI(encodeURI(row.name)) + '">指派</a>';
            } else {
                return '<a class="btn btn-info btn-xs" href="/SubordinateManage/Assignment/GuideAdd?id=' + row.id + '&name=' + encodeURI(encodeURI(row.name)) + '">指派</a> '
    ...
    { field: "d_list_name", title: "经销商名称", align: "center" },
...
function Remove(obj) {
    ...
    top.layer.confirm("您确认要解绑导购员：“" + name + "”吗？", function (index) {
        if ($('[name=__RequestVerificationToken]').length > 0) {
            data["__RequestVerificationToken"] = $('[name=__RequestVerificationToken]').val();
        }
        data["emp_id"] = id;
        $.submitForm({
            url: "/SubordinateManage/Assignment/RemoveGuide?date=" + new Date(),
            param: data,
...
var TableInit = function (isAllGuide) {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/SubordinateManage/Assignment/GuideList?date=" + new Date(),        //请求后台的URL（*）
            ...
            queryParams: function (params) {
                var param = {
                    ...
                    sidx: params.sort,
                    sord: params.order,
                    isAllGuide: $("input[id='ShowAll']").prop("checked"),
                };
                return param;
            },
