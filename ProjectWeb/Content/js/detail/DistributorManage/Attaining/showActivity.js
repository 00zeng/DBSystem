...
$(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            targetContent = data.mainInfo.target_content;
            queryStartTime = data.mainInfo.start_date;
            queryEndTime = data.mainInfo.end_date;
            $("#start_date").val(queryStartTime.substring(0, 10));
            $("#end_date").val(queryEndTime.substring(0, 10));
...
var tableDistributorHeader = [
    ...
    },
    {
        ...
        footerFormatter: (function (value) { return ToThousandsStr(totalCount); })
    },
    {
        field: "total_normal_count", title: "正常机销量（台）", align: "center",
        formatter: (function (value) { return ToThousandsStr(value); }),
        footerFormatter: (function (value) { return ToThousandsStr(totalNormalCount); })
    },
    {
        field: "total_amount", title: "总金额（元）", align: "center",
        formatter: (function (value) { return ToThousandsStr(value); }),
        footerFormatter: (function (value) { return ToThousandsStr(totalAmount); })
    },
    //{
    //    field: "total_ratio", title: "完成率（%）", align: "center", visible: false,
    //    formatter: (function (value) { return ToThousandsStr(total_ratio); })
    //},
    {
        field: "total_rebate", title: "返利金额（元）", align: "center",
        formatter: (function (value) { return ToThousandsStr(value); }),
        footerFormatter: (function (value) { return ToThousandsStr(totalReward); })
    },
]
...
$("#querySubmit").on('click', function () {
    if ($("#startTime1").val() != "") {
        queryStartTime = $("#startTime1").val();
    }
    if ($("#endTime1").val() != "") {
        queryEndTime = $("#endTime1").val();
    }
...
var tableDetailHeader = [
    { field: "name", title: "经销商", align: "center", sortable: true, width: 180 },
...
var TableInit1 = function () {
    ...
    oTableInit.Init = function () {
        $tableDetail.bootstrapTable({
            height: 400,
            url: "/DistributorManage/Attaining/GetSaleList?date=" + new Date(),        //请求后台的URL（*）
            ...
            queryParams: function (params) {
                var param = {
                    ...
                    sord: params.order,
                    id: id,
                    name: $('#distributor_name').val().trim(),
                    start_date: queryStartTime,
                    end_date: queryEndTime,
...
function OpenForm(url, title) {
    //if (title == "修改结束时间")
    url += "?id=" + id + "&start_date=" + queryStartTime + "&end_date=" + queryEndTime;
    $.modalOpen({
        id: "Form",
        title: title,
        url: url,
