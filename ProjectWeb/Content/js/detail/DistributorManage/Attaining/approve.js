        ...
        success: function (data) {
            ...
            function RebateMode1() {
                ...
                $.each(data.rebateList, function (index, item) {
                    ...
                    tr += "<tr style='display: table;width: 100%;table-layout: fixed;'><td><div class='input-group'><input  class='form-control text-center' value='" + item.target_min + "' disabled><span class='input-group-addon no-border'>-</span><input  class='form-control text-center'value='" + item.target_max + "' disabled>"
                        + "<td style='text-align:center;vertical-align:middle;'><div><span>" + item.rebate + "</span>&nbsp;<span name='rebate_mode_input1' >" + str2 + "</span>&nbsp; <span name='rebate_mode_input2' >" + str4 + str5;
                })
            ...
            function ProductScope() {
                ...
                $.each(pSelList, function (index, item) {
                    tr += "<tr style='display: table;width: 100%;table-layout: fixed;'><td style='text-align:center;vertical-align:middle;'>" + item.model + "</td><td style='text-align:center;vertical-align:middle;'>" + item.color + "</td><td style='text-align:center;vertical-align:middle;'>" + item.price_wholesale + "</td><td style='text-align:center;vertical-align:middle;'><div><span class='text-center' name='rebate_p'>" + item.rebate + "</span><span name='rebate_mode_input1'>" + str2 + "</span>&nbsp;<span name='rebate_mode_input2' >" + str4 + "</span></div></td></tr>";
                })
...
function submitForm() {
    ...
    $.submitForm({
        url: "/DistributorManage/Attaining/Approve?date=" + new Date(),
        param: data,
        success: function (data) {
            if (data.state == "success") {
                window.location.href = "/DistributorManage/Attaining/ApproveIndex";
            }
        },
        error: function (data) {
            $.modalAlert(data.responseText, 'error');
