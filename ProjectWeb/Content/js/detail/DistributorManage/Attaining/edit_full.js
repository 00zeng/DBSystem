...
var str5 = "</span></div></td>"
        + "<td class='col-sm-2'><div class='row'><i onclick='addRow(this)' data-index='{1}' name='addRow_{0}' class='fa fa-plus-circle fa-lg  text-blue '></i> <i onclick='deleteRow(this)' name='delRow_{0}' class='fa fa-minus-circle fa-lg text-red'></i></div></td>"
        + "</tr>";
//型号的th
...
var str7 = ' <tr> <td class="col-sm-3"><div class="input-group"><input class="form-control text-center" name="sma_target_min_{0}"  onchange="setNumSmall(this)"><span class="input-group-addon no-border">-</span>'
            ...
            + '<td class="col-sm-1" style="text-align:center;vertical-align:middle"><i onclick="addSmall(this)" name="addSmall_{0}" data-index="{1}" class="fa fa-plus  text-blue "></i>&nbsp;<i onclick="delSmall(this)"  name="delSmall_{0}" class="fa fa-minus   text-red "></i>'
...
var str11 = '<tr> <td class="col-sm-3" style="text-align:center;vertical-align:middle"><div class="input-group"><input class="form-control text-center" name="nor_target_min_{0}"  onchange="setNumNor(this)" >'//
    ...
    + '<i onclick="addSmall(this)" name="addSmall_{0}" data-index="{1}" class="fa fa-plus text-blue "></i>&nbsp;<i onclick="delSmall(this)"  name="delSmall_{0}"  class="fa fa-minus   text-red "></i></td><td class="col-sm-3"><div class="input-group formValue"><input class="form-control text-center required number" name="nor_rebate_{0}">'
    + '<span name="rebate_mode_input1_{0}" class="input-group-addon no-border">';
...
function DateShow() {
    ...
    for (var i = 0; i < proBoxNum; i++) {
        $(proBox[i]).find("input[id^=start_time_]:first").val(start_date);
        $(proBox[i]).find("input[id^=end_time_]:last").val(end_date);
...
$(function () {
    ...
    $.ajax({
        ...
        type: "get",
        data: { id: id, forEdit: true },
        dataType: "json",
        success: function (data) {
            console.log(data);

            var target_sale = data.mainInfo.target_sale;
            start_date = data.mainInfo.start_date.substring(0, 10);
            end_date = data.mainInfo.end_date.substring(0, 10);
            ...
            if (data.mainInfo.money_included == 0) {
                $("#money_included").prop("checked", "checked");
            }
            //返利类型
            ...
            pRestList = data.productRestList;
            ...
            for (var psI = 1; psI < productSecLen; psI++) 
                addProduct();       // 先添加完整的机型分段及“pSelListArr”空数组
            $.each(productList, function (index, value) {
                pSelListArr[value.product_sec_i - 1].push(value);   // 将型号填充到pSelListArr数组中
            });

            // 将未选的型号添加到双向选择框左边
            var duallistProductFirst = $('select[id=duallistProduct1]');
            if (duallistProductFirst.length > 0 && pRestList.length > 0) {
                $.each(pRestList, function (index, value) {
                    $(duallistProductFirst).append("<option value='" + value.id + "'>" + value.model + "(" + value.color + ")" + "</option>");
                })
                duallistProductFirst.bootstrapDualListbox("refresh");
            }

            for (var i = 0; i < productSecLen; i++) {
                if (i > 0 || productSecList[0].product_scope == 2) {    // else “全部机型”时不做处理
                    var pIndex = i + 1;
                    pSelList = pSelListArr[i];
                    $('table[id=targetProduct' + pIndex + ']').bootstrapTable('load', pSelList);
                    $("#product_count" + pIndex + "").text(pSelList.length);
                    $('table[id=targetProduct' + pIndex + ']').show();
                    $("#targetProductWrap" + pIndex).show();
                    $("#productCountWrap" + pIndex).show();
                    $("input[name^='product_scope" + pIndex + "'][value='2']").prop("checked", "checked");
                    $("input[name^='target_mode_" + pIndex + "'][value=4]").removeAttr("disabled");//

                    // 将已选的型号添加到双向选择框右边
                    var curDualListP = $('#duallistProduct' + pIndex);
                    $.each(pSelList, function (index, value) {
                        curDualListP.append("<option selected value='" + value.id + "'>" + value.model + "(" + value.color + ")" + "</option>");
                    })
                    curDualListP.bootstrapDualListbox("refresh");
                    ProIndex = pIndex;
                    ProductScope(1);
                } 
                var product_sec_i = productSecList[i].product_sec_i;
                var k = 0;
                
                for (var j = 0; j < timeSecLen  ; j++) {//时间分段
                    var a = 0;//无
                    var b = 0;//批发价
                    var time_sec_i = timeSecList[j].time_sec_i;
                    var rebate_mode = timeSecList[j].rebate_mode;
                    var target_mode = timeSecList[j].target_mode;
                    if (product_sec_i != timeSecList[j].product_sec_i)
                        continue;
                    k++;
                    if (k > 1) {
                        $("div[id^=copy_" + product_sec_i + "_" + "]").find("span[name=addTimes]").trigger("click");
                    }
                    $("input[name=target_mode_" + product_sec_i + "_" + k + "][value=" + target_mode + "]").trigger("click");
                    $("input[name=rebate_mode_" + product_sec_i + "_" + k + "][value=" + rebate_mode + "]").trigger("click");
                 
                    var cSum = 0;

                    for (var l = 0; l < rebateLen; l++) {
                        var c = 0;//批发价-子table
                        var rebate_i = rebateList[l].rebate_i;
                        if (rebateList[l].time_sec_i != time_sec_i) // else 同一个时间分段
                            continue;
                        if (target_mode == 6) {     // 返利模式：无
                            if (a == 1 && rebateList[l].target_max == -1)
                                $($("i[name=delRow_" + product_sec_i + "_" + k + "]")[0]).trigger("click");
                            else if (a > 1 && rebateList[l].target_max != -1) {
                                $($("i[name=addRow_" + product_sec_i + "_" + k + "]")[a - 1]).trigger("click");
                            }
                            var targetMinList = $("input[name=target_min_" + product_sec_i + "_" + k + "]");//input name相同列表
                            var targetMaxList = $("input[name=target_max_" + product_sec_i + "_" + k + "]");
                            var rebateValList = $("input[name=rebate_" + product_sec_i + "_" + k + "]");//input name相同列表

                            var targetMinLen = targetMinList.length;
                            $(targetMinList[a]).val(rebateList[l].target_min);
                            if (rebateList[l].target_max == -1)
                                $(targetMaxList[a]).val("以上");
                            else
                                $(targetMaxList[a]).val(rebateList[l].target_max);
                            $(rebateValList[a]).val(rebateList[l].rebate);
                        } else if (target_mode == 3 || target_mode == 5) {// 返利模式：批发价/零售价
                            if (b > -1 && rebateList[l].target_max != -1) {
                                $($("i[name=addNormal_" + product_sec_i + "_" + k + "]")[b]).trigger("click");
                            }
                            var targetMinList = $("input[name=nor_target_min_" + product_sec_i + "_" + k + "]");//完成率
                            var targetMaxList = $("input[name=nor_target_max_" + product_sec_i + "_" + k + "]");
                            var targetMinLen = targetMinList.length;
                            $(targetMinList[b]).val(rebateList[l].target_min);
                            if (rebateList[l].target_max == -1)
                                $(targetMaxList[b]).val("以上");
                            else
                                $(targetMaxList[b]).val(rebateList[l].target_max);
                            //-- 完成率区间的部分结束--

                            for (var rp = 0; rp < rebateProLen; rp++) {//小table
                                if (rebateProList[rp].rebate_i != rebate_i)
                                    continue;
                                if (c == 0 && rebateProList[rp].target_max == -1)
                                    $($("i[name=delSmall_" + product_sec_i + "_" + k + "]")[cSum - b]).trigger("click");
                                else if (c > 0 && rebateProList[rp].target_max != -1) {
                                    $($("i[name=addSmall_" + product_sec_i + "_" + k + "]")[cSum + c]).trigger("click");
                                }
                                var targetSmallMinList = $("input[name=sma_target_min_" + product_sec_i + "_" + k + "]");//small
                                var targetSmallMaxList = $("input[name=sma_target_max_" + product_sec_i + "_" + k + "]");
                                var rebateSmallValList = $("input[name=nor_rebate_" + product_sec_i + "_" + k + "]");//input name相同列表
                                var targetSmallMinLen = targetSmallMinList.length;

                                $(targetSmallMinList[cSum + c]).val(rebateProList[rp].target_min);//small - min
                                if (rebateProList[rp].target_max == -1)//small - max
                                    $(targetSmallMaxList[cSum + c]).val("以上");
                                else
                                    $(targetSmallMaxList[cSum + c]).val(rebateProList[rp].target_max);
                                //rebateProList[rp].target_min
                                $(rebateSmallValList[cSum + c]).val(rebateProList[rp].rebate);//small - rebate

                                c++;
                            }
                            cSum += c;

                        } else if (target_mode == 4) {//按型号
                            if (b > -1 && rebateList[l].target_max != -1) {
                                $($("i[name=addTr_" + product_sec_i + "_" + k + "]")[b]).trigger("click");
                            }
                            $("input[name=target_mode_" + product_sec_i + "_" + k + "][value=" + target_mode + "]").trigger("click");
                            
                            var targetProMinList = $("input[name=pro_target_min_" + product_sec_i + "_" + k + "]");//完成率
                            var targetProMaxList = $("input[name=pro_target_max_" + product_sec_i + "_" + k + "]");
                            var targetProMinLen = targetProMinList.length;
                            $(targetProMinList[b]).val(rebateList[l].target_min);
                            if (rebateList[l].target_max == -1)
                                $(targetProMaxList[b]).val("以上");
                            else
                                $(targetProMaxList[b]).val(rebateList[l].target_max);
                            //-- 完成率区间的部分结束--
                            for (var rp = 0; rp < rebateProLen; rp++) {//小table
                                
                                if (rebateProList[rp].rebate_i != rebate_i)
                                    continue;
                                var rebateSmallValList = $("input[name=rebate_p_" + product_sec_i + "_" + k + "]");//input name相同列表
            ...
            //重新循环 给时间赋值
            for (var ps = 0; ps < productSecLen; ps++) {
                var product_sec_i = productSecList[ps].product_sec_i;
                var k = 0;
                for (var ts = 0; ts < timeSecLen ; ts++) {//时间分段
                    if (product_sec_i != timeSecList[ts].product_sec_i)
                        continue;
                    k++;
                    $("input[id=start_time_" + product_sec_i + "_" + k + "]").val(timeSecList[ts].start_date.substring(0, 10));
                    $("input[id=end_time_" + product_sec_i + "_" + k + "]").val(timeSecList[ts].end_date.substring(0, 10));
                }
            }
            if (target_sale == 1) {
...
function addTimes(e) {
    ...
    + '<i onclick="delSmall(this)" name="delSmall_' + ProIndex + '_' + times + '" class="fa fa-minus text-red"></i></td>'
    ...
    + '<i onclick="deleteRow(this)"  name="delRow_' + ProIndex + '_' + times + '" class="fa fa-minus-circle fa-lg text-red"></i></div></td></tr>'
    + '<tr style="display:table;width:100%;table-layout:fixed;">'
    + '<td class="col-sm-6"><div class="input-group">'
    + '<input class="form-control text-center" name="target_min_' + ProIndex + '_' + times + '"  onchange="setNum(this)">'
    + '<span class="input-group-addon no-border">-</span>'
...
function addProduct() {
    ...
    + '<h6 id="productCountWrap' + proCount + '" class="inline margin" style="display:none">共<span class="margin" id="product_count' + proCount + '">0</span>款</h6>'
    ...
    + '</label></div></div>'
    + '<div id="targetProductWrap' + proCount + '" hidden><table id="targetProduct' + proCount + '"></table></div></div></div></div></div>'
    ...
    + '<i onclick="delSmall(this)" name="delSmall_' + proCount + '_1" class="fa fa-minus text-red"></i></td>'
    + '<td class="col-sm-3"><div class="input-group formValue">'
    ...
    + '<span class="input-group-addon no-border">-</span>'
    + '<input class="form-control text-center required number" name="target_max_' + proCount + '_1" onchange="setNum()"></div></td>'
    + '<td class="col-sm-4">'
    + '<div class="input-group formValue">'
    + '<input class="form-control text-center required number" name="rebate_' + proCount + '_1">'
    + '<span name="rebate_mode_input1_' + proCount + '_1" class="input-group-addon no-border">元/台</span>'
...
function Rebate_mode(e) {
    //var tempPIndex = $(e).parents("div[id^=proBox]").data("index")
    //var tempNowIndex = $(e).parents("div[id^=proBox]").data("index")
...
function submitForm() {
    ...
    $.submitForm({
        url: "/DistributorManage/Attaining/EditFull?date=" + new Date(),
        param: data,
        success: function (data) {
