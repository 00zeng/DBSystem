...
function getTotal() {
    $.ajax({
        url: "/DistributorManage/Settlement/GetTotalRefund?distributor_name=" + $('#distributor_name').val() + "&company_id=" + $('#company_id').val()
...
var tableHeader = [
    ...
    //        return "<a class='btn btn-primary btn-xs' href='/DistributorManage/Settlement/RefundShow?id=" + row.id
...
var tableHeader2 = [
    ...
    {
        field: "diff_price", title: "补差金额", align: "center", sortable: true,
        formatter: (function (value) { return ToThousandsStr(value); })
    },
    {
        field: "orig_price", title: "原批发价", align: "center", sortable: true,
        formatter: (function (value) { return ToThousandsStr(value); })
    },
    {
        field: "new_price", title: "新批发价", align: "center", sortable: true,
        formatter: (function (value) { return ToThousandsStr(value); })
    },
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/DistributorManage/Settlement/RefundMainList?date=" + new Date(),        //请求后台的URL（*）
    ...
    oInit.InitSubTable = function (index, row, $detail) {
        ...
        $(cur_table).bootstrapTable({
            url: "/DistributorManage/Settlement/RefundList",        //请求后台的URL（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            sortable: true,                     //是否启用排序
...
//导出 
function Export() {
    window.location.href = "/DistributorManage/Settlement/ExportExcelRefund?company_id=" + $('#company_id').val()
            + "&distributor_name=" + $('#distributor_name').val().trim()
