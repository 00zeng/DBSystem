...
function getTotal() {
    $.ajax({
        url: "/DistributorManage/Settlement/GetTotalRecom?distributor_name=" + $('#distributor_name').val() + "&company_id=" + $('#company_id').val()
...
var tableHeader = [
    ...
    {
        field: "total_rebate", title: "返利总金额", align: "center", sortable: true, width: 120,
        formatter: (function (value) { return ToThousandsStr(value); }),
        footerFormatter: function (value) {
            return String(totalData.total_reward);
        }
    },
    {
        field: "count", title: "返利次数", align: "center", sortable: true, width: 120,
        formatter: (function (value) { return ToThousandsStr(value); }),
        footerFormatter: function (value) {
            return String(totalData.total_activity);
        }
    },
    {
...
var tableHeader2 = [
    {
        ...
        formatter: function (value, row) {
            return "<a class='btn btn-primary btn-xs' href='/DistributorManage/Recommendation/ShowActivity?id=" + row.id + " ' title='查看活动详情'><i class='fa fa-search'></i></a>";
        }
    },
    {
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/DistributorManage/Settlement/RecomMainList?date=" + new Date(),        //请求后台的URL（*）
    ...
    oInit.InitSubTable = function (index, row, $detail) {
        ...
        $(cur_table).bootstrapTable({
            url: "/DistributorManage/Settlement/RecomList",        //请求后台的URL（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            sortable: true,                     //是否启用排序
...
//重新查询  
function Export() {
    window.location.href = "/DistributorManage/Settlement/RecomExport?company_id=" + $('#company_id').val()
            + "&distributor_name=" + $('#distributor_name').val().trim()
