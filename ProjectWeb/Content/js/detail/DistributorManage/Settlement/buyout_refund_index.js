...
function getTotal() {
    $.ajax({
        url: "/DistributorManage/Settlement/GetTotalBuyoutRefund?distributor_name=" + $('#distributor_name').val() + "&company_id=" + $('#company_id').val()
...
var tableHeader2 = [
    ...
    {
        field: "buyout_refund", title: "补差金额", align: "center", sortable: true,
        formatter: (function (value) { return ToThousandsStr(value); })
    },
    {
    ...
    {
        field: "price_buyout", title: "买断价", align: "center", sortable: true,
        formatter: (function (value) { return ToThousandsStr(value); })
    },
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/DistributorManage/Settlement/BuyoutRefundMainList?date=" + new Date(),        //请求后台的URL（*）
    ...
    oInit.InitSubTable = function (index, row, $detail) {
        ...
        $(cur_table).bootstrapTable({
            url: "/DistributorManage/Settlement/BuyoutRefundList",        //请求后台的URL（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            sortable: true,                     //是否启用排序
...
//导出 
function Export() {
    window.location.href = "/DistributorManage/Settlement/ExportExcelBuyoutRefund?company_id=" + $('#company_id').val()
            + "&distributor_name=" + $('#distributor_name').val().trim()
