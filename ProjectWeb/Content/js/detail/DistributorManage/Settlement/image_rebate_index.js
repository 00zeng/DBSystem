...
function getTotal() {
    $.ajax({
        url: "/DistributorManage/Settlement/GetTotalImage?distributor_name=" + $('#distributor_name').val() + "&company_id=" + $('#company_id').val()
...
var tableHeader = [
    ...
    {
        field: "count", title: "返利次数", align: "center", sortable: true, width: 110,
        formatter: (function (value) { return ToThousandsStr(value); }),
        footerFormatter: function (value) {
    ...
    {
        field: "company_name", title: "所属机构", align: "center", sortable: true, width: 150,  
    },
]
...
var tableHeader2 = [
    ...
    {
        field: "pay_mode", title: "返利发放", align: "center", sortable: true, width: 150,
    ...
    {
        field: "target_mode", title: "返利类型", align: "center", sortable: true, width: 150,
        formatter: function (value, row, index) {
            if (value == 1)
                return "按时间返利"
            return "按销量返利"
    ...
    {
        field: "pay_end", title: "返利时间", align: "center", sortable: true, width: 150,
        formatter: function (value, row, index) {
            var startDate = new Date(row.pay_start);
            var endDate = new Date(value);
            return startDate.pattern("yyyy-MM-dd") + ' 至 ' + endDate.pattern("yyyy-MM-dd");
        }
    },
    {
        field: "rebate", title: "返利金额", align: "center", sortable: true, width: 150,
        formatter: (function (value) { return ToThousandsStr(value); })
    },
]
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/DistributorManage/Settlement/ImageMainList?date=" + new Date(),        //请求后台的URL（*）
    ...
    oInit.InitSubTable = function (index, row, $detail) {
        ...
        $(cur_table).bootstrapTable({
            url: "/DistributorManage/Settlement/ImageList",        //请求后台的URL（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            sortable: true,                     //是否启用排序
...
//重新查询  
function Export() {
    window.location.href = "/DistributorManage/Settlement/ImageExport?company_id=" + $('#company_id').val()
            + "&distributor_name=" + $('#distributor_name').val().trim()
