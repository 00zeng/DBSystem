...
var tableHeader = [
    {
        ...
        formatter: function (value, row) {
            return "<a class='btn btn-primary btn-xs' href='/FinancialAccounting/AnnualBonus/Approve?id=" + row.id + "&position_name=" + encodeURI(encodeURI(row.position_name))
                     + "&emp_name=" + encodeURI(encodeURI(row.emp_name)) + "&grade=" + encodeURI(encodeURI(row.grade)) + "'title='审批'><i class='fa fa-check-square-o'></i></a>";
        }
    },
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/FinancialAccounting/AnnualBonus/GetGridJson2?date=" + new Date(),        //请求后台的URL（*）
