...
var tableHeader = [
    {
        ...
        formatter: (function (value, row) {
            ...
            if (position_type == 12) {
                return "<a class='btn btn-primary btn-xs' href='/FinancialAccounting/OfficeKPI/TrainerApprove?id=" + row.id + "&company_name=" + encodeURI(encodeURI(row.company_name)) + "&position_name=" + encodeURI(encodeURI(row.position_name)) + "&name=" + encodeURI(encodeURI(row.name)) + "&grade=" + encodeURI(encodeURI(row.grade)) + "'title='审批'><i class='fa fa-check-square-o'></i></a>";

            } else {
                return "<a class='btn btn-primary btn-xs' href='/FinancialAccounting/OfficeKPI/Approve?id=" + row.id + "&company_name=" + encodeURI(encodeURI(row.company_name)) + "&position_name=" + encodeURI(encodeURI(row.position_name)) + "&name=" + encodeURI(encodeURI(row.name)) + "&grade=" + encodeURI(encodeURI(row.grade)) + "'title='审批'><i class='fa fa-check-square-o'></i></a>";
            }
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/FinancialAccounting/OfficeKPI/GetListApprove?date=" + new Date(),        //请求后台的URL（*）
