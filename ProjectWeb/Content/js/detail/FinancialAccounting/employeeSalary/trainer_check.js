...
var tableHeader = [

 {
     ...
     formatter: (function (value, row) {
         return "<a class='btn btn-primary btn-xs' href='/FinancialAccounting/EmployeeSalary/TrainerCheckSetting?id="
                    + row.id + "&company_name=" + encodeURI(encodeURI(row.company_name)) + "&position_name=" + encodeURI(encodeURI(row.position_name)) + "&entry_date=" + row.entry_date
                    + "&name=" + encodeURI(encodeURI(row.name)) + "&grade=" + encodeURI(encodeURI(row.grade)) + "&emp_category=" + encodeURI(encodeURI(row.emp_category))
                    + "&position_type=" + row.position_type + "'title='考核设置'><i class='fa fa-edit'></i></a>";
     })
 },
    {
    ...
    { field: "salary_category", title: "导购员", align: "center", sortable: true, width: 100 },
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/FinancialAccounting/EmployeeSalary/GetTrainerKPIList?date=" + new Date(),        //请求后台的URL（*）
