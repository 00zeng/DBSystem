...
var tableHeader = [
    {
        ...
        formatter: (function (value, row) {
            return "<a class='btn btn-primary btn-xs' href='/FinancialAccounting/EmployeeSalary/TrainerSetting?id=" + row.id + "&company_name=" + encodeURI(encodeURI(row.company_name))
                        + "&position_name=" + encodeURI(encodeURI(row.position_name)) + "&entry_date=" + row.entry_date
                        + "&name=" + encodeURI(encodeURI(row.name)) + "&grade=" + encodeURI(encodeURI(row.grade)) + "&emp_category=" + encodeURI(encodeURI(row.emp_category))
                        + "'title='薪资设置'><i class='fa fa-edit'></i></a>";
        })
    },
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/FinancialAccounting/EmployeeSalary/GetTrainerList?date=" + new Date(),        //请求后台的URL（*）
