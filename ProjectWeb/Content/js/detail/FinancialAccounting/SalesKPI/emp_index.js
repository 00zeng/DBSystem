...
var tableHeader = [
    {
        ...
        formatter: (function (value, row) {
            return "<a class='btn btn-primary btn-xs' href='/FinancialAccounting/SalesKPI/Add?id=" + row.id
                + "&company_name=" + encodeURI(encodeURI(row.company_name)) + "&position_name=" + encodeURI(encodeURI(row.position_name))
                + "&name=" + encodeURI(encodeURI(row.name)) + "&grade=" + encodeURI(encodeURI(row.grade))
                + "'title='业务KPI核对'><i class='fa fa-edit'></i></a>";
        })
    },
    {
    ...
    { field: "name_v2", title: "v2姓名", align: "center", sortable: true, width: 100 },
    {
        ...
        formatter: (function (value) {
            if (value == null)
                return "<span class='label label-warning'>当月未核对</span>";
            else
                return "<span class='label label-primary'>当月已核对</span>";
        })
    },
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/FinancialAccounting/SalesKPI/GetListCalculate?date=" + new Date(),        //请求后台的URL（*）
