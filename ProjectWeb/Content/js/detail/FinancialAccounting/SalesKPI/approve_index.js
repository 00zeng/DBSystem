...
var tableHeader = [
    {
        ...
        formatter: (function (value, row) {
            ...
            if (position_type == 12) {
                return "<a class='btn btn-primary btn-xs' href='/FinancialAccounting/KPI/TrainerApprove?id=" + row.id + "&company_name=" + encodeURI(encodeURI(row.company_name)) + "&position_name=" + encodeURI(encodeURI(row.position_name)) + "&name=" + encodeURI(encodeURI(row.name)) + "&grade=" + encodeURI(encodeURI(row.grade)) + "'title='审批'><i class='fa fa-check-square-o'></i></a>";

            } else {
                return "<a class='btn btn-primary btn-xs' href='/FinancialAccounting/KPI/Approve?id=" + row.id + "&company_name=" + encodeURI(encodeURI(row.company_name)) + "&position_name=" + encodeURI(encodeURI(row.position_name)) + "&name=" + encodeURI(encodeURI(row.name)) + "&grade=" + encodeURI(encodeURI(row.grade)) + "'title='审批'><i class='fa fa-check-square-o'></i></a>";
            }
    ...
    { field: "grade", title: "等级", align: "center", sortable: true },
    { field: "emp_category", title: "雇员等级", align: "center", sortable: true },
    { field: "category", title: "类型", align: "center", sortable: true },
    { field: "area_name", title: "所属区域", align: "center", sortable: true },
    { field: "dept_name", title: "所属部门", align: "center", sortable: true },
    { field: "company_linkname", title: "所属机构", align: "center", sortable: true },
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/FinancialAccounting/KPI/GetGridJson2?date=" + new Date(),        //请求后台的URL（*）
