...
var tableHeader = [
    {
        ...
        formatter: function (value, row) {
            return "<a class='btn btn-primary btn-xs' href='/FinancialAccounting/Rewards/Add?id=" + row.id
                   + "&name=" + encodeURI(encodeURI(row.name))
                   + "&grade=" + encodeURI(encodeURI(row.grade))
                   + "&position_name=" + encodeURI(encodeURI(row.position_name))
                   + "&company_name=" + encodeURI(encodeURI(row.company_name))
                   + "&month=" + row.month + "'title='员工奖罚设置'><i class='fa fa-edit'></i></a>";
    ...
    { field: "area_l1_name", title: "经理片区", align: "center", sortable: true },
    { field: "area_l2_name", title: "业务片区", align: "center", sortable: true },
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            height: h,
            url: "/FinancialAccounting/Rewards/GetGridJson3?date=" + new Date(),        //请求后台的URL（*）
            ...
            //    location.href = "/FinancialAccounting/Rewards/Add?id=" + row.id + "&name=" + encodeURI(encodeURI(row.name)) + "&grade=" + encodeURI(encodeURI(row.grade)) + "&position_name=" + encodeURI(encodeURI(row.position_name)) + "&company_name=" + encodeURI(encodeURI(row.company_name));
