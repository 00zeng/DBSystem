(function ($) {
    ...
    $.fn.authTree = function (settings) {
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            if (nd.hasChildren || nd.parentId=="0") {
                cs.push(nd.isexpand ? "bbit-tree-node-expanded" : "bbit-tree-node-collapsed");
            }
            else {
                cs.push("bbit-tree-node-leaf last_lavel");
            }
            ...
            if (dfop.showcheck && nd.showcheck) {
                ...
                ht.push("<img  id='", id, "_", nid, "_cb' class='bbit-tree-node-cb' disabled='" + nd.disabled + "' data-category='" + nd.category + "' hasModel='" + nd.hasModel + "' src='", dfop.cbiconpath, dfop.icons[nd.checkstate], "'/>");
            }
            ...
            //if(nd.parentId=="0"){
                ...
                if (nd.img == -1) {
                    ...
                    if (!!nd.img) {
                        ...
                        if (nd.category == 1)
                            ht.push("<i class=\"fa fa-folder-o\"></i>");
                        else
                            ht.push("<i class=\"fa fa-file-text-o\"></i>");
                    }
            }
            ht.push("<a hideFocus class='bbit-tree-node-anchor' tabIndex=1 href='javascript:void(0);'>");
            ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' " + (nd.disabled ? "style='color:gray !important;'" : "") + " unselectable='on'>", nd.text, "</span>");
            ht.push("</a>");
            ...
            if (nd.hasChildren ) {
                    if (nd.isexpand) {
                    ht.push("<ul  class='bbit-tree-node-ct'  style='z-index: 0; position: static; visibility: visible; top: auto; left: auto; overflow:hidden; zoom:1;'>");
                    if (nd.ChildNodes) {
        ...
        function nodeclick(e) {
            ...
            if (et.tagName == "IMG") {
                ...
                if ($(et).hasClass("bbit-tree-node-text")) {
                    $(et).parent().siblings(".bbit-tree-node-cb").trigger("click");
                }
                dfop.citem = item;
